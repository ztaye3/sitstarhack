...
import {ADD_USER_URL, GET_USER_URL, HOME_URL, SIGNUP_URL, USER_PROFILE_URL, USER_URL} from "../../../utils/Constant";
...
import {
    ...
    DELETE_USER_SUCCESS,
    GET_ALL_USER_FAIL,
    GET_ALL_USER_SUCCESS, SEARCH_CONNECTION_FAIL, SEARCH_CONNECTION_SUCCESS,
    SEARCH_DATA__DETAIL_SUCCESS,
    SEARCH_DATA_SUCCESS,
    SEARCH_KEY_FAIL,
    SEARCH_KEY_SUCCESS,
    UPDATE_USER_FAIL,
...
export const getData = (userInput) => {
    ...
    return function (dispatch){
        if(userInput.toString().includes("cat")){

            dispatch({
                            type: SEARCH_KEY_SUCCESS,
                        })
                    axios
            .get("http://127.0.0.1:8000/cats/images?token=" + localStorage.getItem("access"))
            .then(async response => {
                ...
                axios
                    .get("http://127.0.0.1:8000/cats/?token=" + localStorage.getItem("access"))
                    .then(async response => {

                        const data = response.data;

                        dispatch({
                            type: SEARCH_DATA__DETAIL_SUCCESS,
                            payload: data
                        })
